...
function hasSelectedConstructionBuildingAndIsAllowed(x, y) {
...
function hasSelectedUnitAndIsInMoveRange(x, y) {
...
function drawMap() {
	for (let y = Math.max(0, topLeftVisible.y); y < Math.min(map.length, bottomRightVisible.y); y++) {
		for (let x = Math.max(0, topLeftVisible.x); x < Math.min(map[0].length, bottomRightVisible.x); x++) {
			if (map[y][x].displayType != 0) {
				...
				if (hasSelectedConstructionBuildingAndIsAllowed(x, y) ||
					hasSelectedUnitAndIsInMoveRange(x, y)) {
					drawImage(resources[RESOURCE_GREEN_OVERLAY],
						(x * 96),
						(y * 111) + yOffset);
				}
				...
				if (currentScreen == GAME_SCREEN && state.gameState.mapObjects[y][x]) {
					...
					if (state.gameState.mapObjects[y][x].turnsUntilBuilt == 0) {
						if (name in structures) {
							drawImage(structures[name].image, (x * 96), (y * 111) + yOffset);
						}
						else if (name in units) {
							if (objectOnMySide(state.gameState.mapObjects[y][x])) {
								drawImage(resources[RESOURCE_GREEN_OVERLAY], (x * 96), (y * 111) + yOffset);
							}
							else {
								drawImage(resources[RESOURCE_RED_OVERLAY], (x * 96), (y * 111) + yOffset);
							}
							drawImage(units[name].image, (x * 96), (y * 111) + yOffset);
						}
					}
					else {
						if (state.gameState.mapObjects[y][x].width == 0) {
							drawImage(resources[RESOURCE_WIDTH_0_BUILD], (x * 96), (y * 111) + yOffset);
						}
						else {
							drawImage(resources[RESOURCE_WIDTH_1_BUILD], (x * 96), (y * 111) + yOffset);
						}
					}
					context.shadowBlur = 0;
				}
	...
	if (state.buildingStructure) {
		...
		for (let i = 0; i < surrounding.length; i++) {
			if (withinMap(surrounding[i])) {
				if (state.allowedToBuildOrSpawn) {
					state.cursorMessage = "Build " + state.buildingStructure;
				}
				else {
					state.cursorMessage = "Cannot build there!";
				}
		...
		if (withinMap(mouseState.tile)) {
			if (state.allowedToBuildOrSpawn) {
				state.cursorMessage = "Spawn " + state.spawningUnit;
			}
			else {
				state.cursorMessage = "Cannot spawn there!";
			}
			drawImage(resources[state.allowedToBuildOrSpawn ? RESOURCE_GREEN_OVERLAY : RESOURCE_RED_OVERLAY],
...
function drawText(text, color, fontSize, x, y, align = "left", fontStyle = "", maxWidth = 0, fontFamily = "Roboto Slab") {
...
function drawUI() {
	if (state.bigMessage) {
		...
		let measure = context.measureText(state.bigMessage);
		let width = measure.width + 30;
		let height = 70;
	...
	// Gold
	context.textBaseline = "middle";
	drawText(getGold(), "white", 32, screenWidth - 185, screenHeight - 184, "left", "bold");
	context.textBaseline = "alphabetic";
	...
	if (state.selectedObject) {
		...
		if (state.selectedObject.turnsUntilBuilt == 0) {
			var lineHeight = 16;
			...
			let t = state.selectedObject.turnsUntilBuilt;
			drawText("Constructing...(" + t + " turn" + (t != 1 ? "s" : "") + " left!)", "black", 13, 88,
				screenHeight - resources[RESOURCE_UI_BOTTOM_LEFT].height + 130, "center", "bold");
		}

		state.hoveredOption = null;
		if (objectOnMySide(state.selectedObject) && state.selectedObject.turnsUntilBuilt == 0) {
	...
	if (state.cursorMessage) {
		...
		let measure = context.measureText(state.cursorMessage);
		let width = measure.width + 10;
		let height = 20;

		drawRectangle("black",
			mouseState.position.x - (width / 2),
			mouseState.position.y - 30,
			width, height);

		context.textBaseline = "middle";
		drawText(state.cursorMessage, "white", 14,
			mouseState.position.x,
			mouseState.position.y + height / 2 - 30, "center");
		context.textBaseline = "alphabetic";
	}
