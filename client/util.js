...
function toCameraCoord(position) {
    return createTuple(
        (position.x - state.camera.position.x) * state.camera.scale + screenWidth / 2,
        (position.y - state.camera.position.y) * state.camera.scale + screenHeight / 2
    );
}

function toWorldCoord(position) {
    return createTuple(
        (position.x - screenWidth / 2) / state.camera.scale + state.camera.position.x,
        (position.y - screenHeight / 2) / state.camera.scale + state.camera.position.y
    );
}
